...
def vip_agent_node(state: SupportState):
	...
	response = AIMessage(
        content=(
            "Hello, valued VIP customer. You have been routed to our "
            "priority support lane. A senior agent will assist you "
            "immediately."
        )
    )
	return {
		"messages": [response],
        "issue_type": "vip_priority",
...
def standard_agent_node(state: SupportState):
	...
	response = AIMessage(
        content=(
            "Thanks for reaching out. Your request is being reviewed and "
            "may be escalated to a specialist if it requires deeper "
            "attention."
        )
    )
	return {
		"messages": [response],
        "issue_type": "standard_request",
